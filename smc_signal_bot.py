...
def calc_atr(candles, p=14):
    ...
    return sum(trs[-p:]) / min(p, len(trs)) iftrs else 0
...
def get_structure(candles, lb=5):
    ...
    choch = ("BEARISH_CHOCH" if trend=="BULLISH" and price<pl else"BULLISH_CHOCH" if trend=="BEARISH" and price>ph else None)
...
def analyze(symbol):
    ...
    bull_ob, bear_ob   = find_obs(itf)bull_fvg,bear_fvg  = find_fvg(itf)
    ...
    for d in ["LONG","SHORT"]:
        ...
        # Manipulation
        if d=="LONG"  andmanip=="BULLISH_MANIP": score+=15; conf+=1
