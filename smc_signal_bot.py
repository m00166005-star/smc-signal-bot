...
def detect_session():
    ...
    if 0 <= hour < 7:
        return "ASIAN", "آسیا - تجمع الگوریتمی"
    elif 7 <= hour < 12:
        return "LONDON", "لندن - دستکاری نهادی"
    elif 12 <= hour < 20:
        return "NEWYORK", "نیویورک - بالاترین نوسان"
    else:
...
def score_signal(symbol, direction):
    ...
    if direction == "LONG" and htf_struct == "BULLISH":
        ...
        warnings.append("HTF بی‌روند - احتیاط!")
    else:
    ...
    for ob in reversed(itf_obs):
        if direction == "LONG" and ob["type"] == "BULLISH_OB":
            if ob["bottom"] <= price <= ob["top"] * 1.01:
                score += 15
                reasons.append(f"روی Bullish OB ({ob['bottom']:.4f}-{ob['top']:.4f})")
                ob_hit = True
                active_ob = ob
                break
            ...
            if ob["bottom"] * 0.99 <= price <= ob["top"]:
                score += 15
                reasons.append(f"روی Bearish OB ({ob['bottom']:.4f}-{ob['top']:.4f})")
                ob_hit = True
                active_ob = ob
                break
    if not ob_hit:
    ...
    if session in ["LONDON", "NEWYORK"]:
        ...
        warnings.append(f"سشن {session_desc}")
    extras = {
...
📐 R:R = 1:{e['rr']:.1f}"""
    ...
    if sig["warnings"]:
        ...
        for w in sig["warnings"][:2]:
            msg += f"\n• {w}"
    msg += f"\n─────────────────"
    msg += f"\n🕐 {datetime.now().strftime('%H:%M')} | ⚠️ _صرفاً آموزشی_"
    return msg
...
def main():
    print("SMC Signal Bot - Single Run (GitHub Actions)")

    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print("خطا: TELEGRAM_TOKEN یا TELEGRAM_CHAT_ID تنظیم نشده (Secrets را بررسی کن)")
        return

    now = datetime.now(timezone.utc)
    print(f"Run time: {now.strftime('%Y-%m-%d %H:%M:%S')} UTC")

    # پیام تستی - فقط وقتی دستی با test_message=true اجرا شود
    if os.environ.get("SEND_TEST_MESSAGE") == "true":
        send_telegram("✅ تست موفق - بات از GitHub Actions به تلگرام وصل شد")

    # خلاصه بازار فقط یک‌بار در ساعت (نزدیک به دقیقه‌ی صفر هر اجرا)
    if now.minute < CHECK_INTERVAL_MINUTES:
        send_market_summary()
